...
__global__ void kernel_random_sampler(IndexType* GPU_src_index, IdType** node_adjmatrix, IdType** edge_adjmatrix, IdType* all_col_size,
...
public:
  ...
  /*   Status Sample( const SamplingRequest* req,   SamplingResponse* res ) override { */
    void Sample(SamplingRequest* req) {			
		...
		//sampling
		kernel_random_sampler<<<kernel_sampling_block,kernel_sampling_thread>>>(d_src_indexs, node_adjmatrix, edge_adjmatrix, all_col_size, batch_size, count,  GPU_dst_nodes_ids, GPU_dst_edges_ids);
		cudaDeviceSynchronize();

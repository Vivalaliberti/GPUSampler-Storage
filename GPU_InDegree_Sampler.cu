...
__global__ void kernel_get_neighbors_indegrees(IdType* neighbor_id_buffer, IndexType* neighbor_indegree_buffer, IndexType* all_neighbor_indegrees, IdType buffer_size){
	int ix = blockDim.x * blockIdx.x + threadIdx.x;
	int iy = blockDim.y * blockIdx.y + threadIdx.y;
	int idx = ix + (gridDim.x * blockDim.x) * iy;
	if (idx < buffer_size){
		neighbor_indegree_buffer[idx] = all_neighbor_indegrees[neighbor_id_buffer[idx]];
	}
}
/*
...
__global__ void kernel_get_cdf(IndexType* neighbor_indegree_buffer, IndexType* partial_sums, IdType* neighbor_position_buffer, float* neighbor_cdf_buffer, IdType buffer_size){
	int ix = blockDim.x * blockIdx.x + threadIdx.x;
	int iy = blockDim.y * blockIdx.y + threadIdx.y;
	int idx = ix + (gridDim.x * blockDim.x) * iy;
	if (idx < buffer_size){
		neighbor_cdf_buffer[idx] = neighbor_indegree_buffer[idx] * 1.0 / partial_sums[neighbor_position_buffer[idx]];
	}
}
...
__global__ void kernel_get_all_dst_ids(IdType* neighbor_position_buffer, float* neighbor_cdf_buffer, IdType* neighbor_id_buffer,
	...
	if (idx < buffer_size - 1){
		for(int i = 0; i < count; i++){
			...
			if (neighbor_cdf_buffer[idx] <= random_number && (neighbor_cdf_buffer[idx + 1] > random_number || neighbor_cdf_buffer[idx + 1] == 0)){
				GPU_dst_nodes_ids[position * count + i] = neighbor_id_buffer[idx];
			}
		}
		...
		for(int i = 0; i < count; i++){
			IdType position = neighbor_position_buffer[idx];
			float random_number = random_number_buffer[position * count + i];
			if (neighbor_cdf_buffer[idx] <= random_number){
				GPU_dst_nodes_ids[position * count + i] = neighbor_id_buffer[idx];
			}
		}
	}
...
class GPUInDegreeSampler /* : public Sampler  */{
public:
	virtual ~GPUInDegreeSampler() {}
  ...
  /*   Status Sample( const SamplingRequest* req,   SamplingResponse* res ) override { */
    void Sample(SamplingRequest* req) {		
		...
		IndexType* neighbor_indegree_buffer;
		cudaMallocManaged(&neighbor_indegree_buffer, buffer_size * sizeof(IndexType));
		cudaMemset(neighbor_indegree_buffer, 0, buffer_size * sizeof(IndexType));
		cudaDeviceSynchronize();
		...
		kernel_get_neighbors_ids<<<kernel_get_neighbors_ids_block, kernel_get_neighbors_ids_thread>>>(d_src_indexs, neighbors_sizes_list, node_adjmatrix, 
		...
		IndexType* all_node_indegrees = storage_ -> GetAllInDegrees();

		dim3 kernel_get_neighbors_degrees_block(buffer_size / (buffer_size_grained_thread_num * buffer_size_grained_thread_num) + 1, 1);
		dim3 kernel_get_neighbors_degrees_thread(buffer_size_grained_thread_num, buffer_size_grained_thread_num);
		kernel_get_neighbors_indegrees<<<kernel_get_neighbors_degrees_block, kernel_get_neighbors_degrees_thread>>>(neighbor_id_buffer, neighbor_indegree_buffer,
																																	 all_node_indegrees, buffer_size);
		cudaDeviceSynchronize();

		//calculate the partial sum and partial prefix sums

		IndexType* partial_sums;
		cudaMallocManaged(&partial_sums, batch_size * sizeof(IndexType));
		cudaMemset(partial_sums, 0, batch_size * sizeof(IndexType));
		...
		for(int idx = 0; idx < batch_size; idx++){
			if (idx < batch_size - 1){
				partial_sums[idx] = thrust::reduce(neighbor_indegree_buffer + cumulative_size_list[idx], neighbor_indegree_buffer + cumulative_size_list[idx + 1]);
				thrust::exclusive_scan(thrust::device, neighbor_indegree_buffer + cumulative_size_list[idx], neighbor_indegree_buffer + cumulative_size_list[idx + 1], neighbor_indegree_buffer + cumulative_size_list[idx]);
			}else if(idx == batch_size - 1){
				partial_sums[idx] = thrust::reduce(neighbor_indegree_buffer + cumulative_size_list[idx], neighbor_indegree_buffer + buffer_size);
				thrust::exclusive_scan(thrust::device, neighbor_indegree_buffer + cumulative_size_list[idx], neighbor_indegree_buffer + buffer_size, neighbor_indegree_buffer + cumulative_size_list[idx]);
			}
		}
		cudaDeviceSynchronize();

		//get the cdf 
		float* neighbor_cdf_buffer;
		cudaMallocManaged(&neighbor_cdf_buffer, buffer_size * sizeof(float));
		cudaMemset(neighbor_cdf_buffer, 0, buffer_size * sizeof(float));
		...
		kernel_get_cdf<<<kernel_get_cdf_block, kernel_get_cdf_thread>>>(neighbor_indegree_buffer, partial_sums, neighbor_position_buffer, neighbor_cdf_buffer, buffer_size);
		cudaDeviceSynchronize();
		for(int i = 0; i < buffer_size; i++){
			std::cout<<"p "<<neighbor_position_buffer[i]<<" ";
			std::cout<<"d "<<neighbor_indegree_buffer[i]<<" ";
			std::cout<<"i "<<neighbor_id_buffer[i]<<" ";
			std::cout<<"c "<<neighbor_cdf_buffer[i]<<"\n";
		}
		...
		//sampling
		kernel_get_all_dst_ids<<<kernel_get_all_dst_ids_block, kernel_get_all_dst_ids_thread>>>(neighbor_position_buffer, neighbor_cdf_buffer, neighbor_id_buffer,
		...
		cudaFree(neighbor_indegree_buffer);
		cudaFree(neighbor_id_buffer);
		cudaFree(neighbor_cdf_buffer);
